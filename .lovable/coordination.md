...
| W6 | Dogfood loop — run candidates back through v1 Simulator (`simulate-idea` → `persona-perspective` → `synthesize`) for a deeper read | Claude Code or Lovable | **PROPOSAL** | Cheap to wire: pass `prefillIdea` (already supported) into the simulator from a candidate, store the result with a `signal_candidate_id` foreign key for traceability. |
...
| W8 | `cluster_id` integrity check on `feature_candidates` (confirm every open candidate has it populated; backfill if not) | Claude Code | **OPEN** | Lovable's evidence drawer assumes `feature_candidates.cluster_id` → `signal_raw.cluster_id`. Confirm + backfill if needed. |
